 ...
 *  Portions Copyright (C) "The PomPP research team" supported by the JST, 
...
#include "src/plugins/select/linear/select_linear_power.h"
...
static uint32_t pparse_dram_package(char *comment);
...
// 2015 06 15 sakamoto add int get_rapl_package(*char comment)
/*
 write comment
*/
static uint32_t pparse_rapl_package(char *comment){
...
//      return -1;
    ...
    //buff = malloc(sizeof(char) * strlen(comment));
...
// 2015 06 15 sakamoto add int get_rapl_dram(*char comment)
/*
 write comment
*/
...
uint32_t tmp_total_power = 0;
  ...
  debug3("   use package %u", job_ptr->cpu_cnt/8);
  debug3("   package %u",pparse_rapl_package(job_ptr->comment));
  debug3("   dram %u",pparse_rapl_dram(job_ptr->comment));
  debug3("   job total w %u", (job_ptr->cpu_cnt/8) * (pparse_rapl_package(job_ptr->comment) + pparse_rapl_dram(job_ptr->comment)));
  debug3("   --------------------");
  for(i = 0; i<cr_ptr->run_job_len; i++){
    ...
    debug3(" preb job id %u", tmp_job->job_id);
      if(tmp_job->comment){
        ...
        debug3("     use package %u", tmp_job->cpu_cnt/8);
        debug3("     dram    %u", pparse_rapl_dram(tmp_job->comment));
        
        debug3("     job_total w %u",  tmp_job->cpu_cnt/8  * (pparse_rapl_package(tmp_job->comment) + pparse_rapl_dram(tmp_job->comment)));
        tmp_total_power += (tmp_job->cpu_cnt/8)*pparse_rapl_package(tmp_job->comment);
        tmp_total_power += (tmp_job->cpu_cnt/8)*pparse_rapl_dram(tmp_job->comment);
      }
    }
  }
  ...
  if(job_ptr->comment){
    tmp_total_power += (job_ptr->cpu_cnt/8) * (pparse_rapl_package(job_ptr->comment) + pparse_rapl_dram(job_ptr->comment));
  }
...
extern int init ( void )
{
  ...
  power_budget = TOTAL_POWER_LIMIT;
  #ifdef EN_POWERLIMIT
