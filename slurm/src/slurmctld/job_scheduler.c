...
#include "src/common/pw_info.h"  // cao added
...
extern double pw_remain;
extern double pw_one_cpu_socket;
...
next_part:		part_ptr = (struct part_record *)
		...
		if (error_code == SLURM_SUCCESS) {
			last_job_update = now;
         info("sched: [replace_batch_job(){if (error_code == SLURM_SUCCESS)}");
...
extern int schedule(uint32_t job_limit)
{
   int caolog = 0;
   info("=====================");
   info("job_scheduler.c, starting int schedule(uint32_t job_limit = %u)", job_limit);
	ListIterator job_iterator = NULL, part_iterator = NULL;
	List job_queue = NULL;
	...
	if (fifo_sched) {
      ...
      if (caolog) printf("cao:   job_scheduler.c, int schedule(), build_job_queue(), then sort_job_queue(job_queue), slurmctld_diag_stats.schedule_queue_len = '%i'\n", slurmctld_diag_stats.schedule_queue_len);      
	...
	while (1) {
      lt = time(NULL); ptr = localtime(&lt);  strftime(strTimeNow, 255, "%Y%m%d:%H:%M:%S", ptr);  gettimeofday(&nowt, NULL);
      sprintf(strTimeNow, "%s.%i", strTimeNow, (int)(1000000*((double) nowt.tv_usec/1000000)));
      //sprintf(strTimeNow, "%s.%5f", strTimeNow, (double) nowt.tv_usec/1000000);      
   
      // info("cao: job_scheduler.c, int schedule(), stating the while(1) at %s, fifo_sched = '%i'", strTimeNow, fifo_sched);
		if (fifo_sched) {
         // info("cao: job_scheduler.c, int schedule(), if (fifo_sched)");
			if (job_ptr && part_iterator &&
...
next_part:			part_ptr = (struct part_record *)
         ...
         // info("cao: job_scheduler.c, int schedule(), fifo_sched=FALSE (if (fifo_sched) else)");
      ...
      // info("cao: job_scheduler.c, int schedule(), line 1102");
...
next_task:
      // info("cao: job_scheduler.c, int schedule(), line 1112, next_task:");
		...
		if ((job_ptr->array_task_id != NO_VAL) || job_ptr->array_recs) {
         // info("cao: job_scheduler.c, int schedule(), line 1123, if ((job_ptr->array_task_id != NO_VAL) || job_ptr->array_recs)");
		...
		if (max_jobs_per_part) {
         // info("cao: job_scheduler.c, int schedule(), line 1140, if (max_jobs_per_part)");
			bool skip_job = false;
		...
		if (job_ptr->resv_name) {
         // info("cao: job_scheduler.c, int schedule(), line 1177, if (job_ptr->resv_name)");
		...
		/* Test for valid account, QOS and required nodes on each pass */
		if (job_ptr->state_reason == FAIL_ACCOUNT) {
         // info("cao: job_scheduler.c, int schedule(), line 1217, if (job_ptr->state_reason == FAIL_ACCOUNT)");
			slurmdb_association_rec_t assoc_rec;
			memset(&assoc_rec, 0, sizeof(slurmdb_association_rec_t));
			assoc_rec.acct      = job_ptr->account;
			if (job_ptr->part_ptr)
		...
		if (job_ptr->qos_id) {
         // info("cao: job_scheduler.c, int schedule(), line 1244, if (job_ptr->qos_id)");
			slurmdb_association_rec_t *assoc_ptr;
		...
		if (!acct_policy_job_runnable_state(job_ptr) &&
          ...
          {
             // info("cao: job_scheduler.c, int schedule(), line 1270 continue");
			    continue;
          }
		...
		if ((job_ptr->state_reason == WAIT_NODE_NOT_AVAIL) &&
		    ...
		    !bit_super_set(job_ptr->details->req_node_bitmap,
				   avail_node_bitmap)) {
         // info("cao: job_scheduler.c, int schedule(), line 1277 continue");
			continue;
		}
		...
		if ((job_ptr->details &&
			...
			debug3("sched: JobId=%u. State=%s. Reason=%s. "
			       ...
			       job_ptr->priority,
			       job_ptr->partition);
         // info("cao: job_scheduler.c, int schedule(), line 1300 continue");
			continue;
		}
		if (license_job_test(job_ptr, time(NULL)) != SLURM_SUCCESS) {
			...
			debug3("sched: JobId=%u. State=%s. Reason=%s. "
			       ...
			       job_reason_string(job_ptr->state_reason),
			       job_ptr->priority);
         // info("cao: job_scheduler.c, int schedule(), line 1312 continue");
			continue;
		}
		...
		if (assoc_mgr_validate_assoc_id(acct_db_conn,
			...
			job_ptr->state_reason = FAIL_ACCOUNT;
			xfree(job_ptr->state_desc);
         // info("cao: job_scheduler.c, int schedule(), line 1328 continue");
			continue;
		}

      if (caolog) printf("cao:   jobscheduler.c, before select_nodes() JobId=%u NodeList=%s #CPUs=%u at %s\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);            
		
      if (caolog)  printf("cao:   jobscheduler.c, int schedule(), Give the backfilling to allocate the job\n");   
      printf("                                     Commented the 'error_code = select_nodes(job_ptr...);'\n");   
      printf("                                     And force error_code = ESLURM_NODES_BUSY here\n");   
      ...
      sprintf(strTimeNow, "%s.%i", strTimeNow, (int)(1000000*((double) nowt.tv_usec/1000000)));
		...
		if (caolog) printf("cao:   jobscheduler.c, after select_nodes() JobId=%u NodeList=%s #CPUs=%u at %s\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);            
      
		if (error_code == ESLURM_NODES_BUSY) {
         if (caolog) printf("slurmctld/job_scheduler.c, error_code == ESLURM_NODES_BUSY when do select_nodes(), JobId=%u\n", job_ptr->job_id);
         FILE *f = fopen("/tmp/cao_slurm.log", "a+");
         fprintf(f, "%s   jobscheduler.c      (int schedule().                 JobId=%u, #CPUs=%u, error_code == ESLURM_NODES_BUSY\n", strTimeNow, job_ptr->job_id, job_ptr->total_cpus); 
         if (caolog) printf("%s   jobscheduler.c      (int schedule().                 JobId=%u, #CPUs=%u, error_code == ESLURM_NODES_BUSY\n", strTimeNow, job_ptr->job_id, job_ptr->total_cpus); 
         fclose(f);         
			debug3("sched: JobId=%u. State=%s. Reason=%s. "
			       "Priority=%u. Partition=%s.",
			       job_ptr->job_id,
         ...
         if (caolog) printf("cao:   job_scheduler.c, line 1416, error_code == SLURM_SUCCESS, job initiated\n");
			/* job initiated */
...
#else
			if (job_ptr->array_task_id != NO_VAL) {
            info("sched: [extern int schedule(uint32_t job_limit){job_ptr->array_task_id != NO_VAL}]");
				info("sched: job_scheduler.c, int schedule line 1438, Allocate JobId=%u_%u (%u) "
				     "NodeList=%s #CPUs=%u",
				     job_ptr->array_job_id,
				     job_ptr->array_task_id,
				     job_ptr->job_id, job_ptr->nodes,
            ...
            sprintf(strTimeNow, "%s.%i", strTimeNow, (int)(1000000*((double) nowt.tv_usec/1000000)));
				...
				info("cao: jobscheduler.c, line 1489, JOB_SCHEDULER ALLOCATED JobId=%u NodeList=%s #CPUs=%u at %s",
				     job_ptr->job_id, job_ptr->nodes,
				     job_ptr->total_cpus, strTimeNow);
            FILE *f = fopen("/tmp/cao_slurm.log", "a+");
            printf("%s   jobscheduler.c      (int schedule).                  JobId=%u, #CPUs=%u, job is allocated\n", strTimeNow, job_ptr->job_id, job_ptr->total_cpus);
            fprintf(f, "%s   jobscheduler.c      (int schedule).                  JobId=%u, #CPUs=%u, job is allocated\n", strTimeNow, job_ptr->job_id, job_ptr->total_cpus);
            fclose(f);
         ...
         if (caolog) printf("cao:   jobscheduler.c, line 1499, rebuild_job_part_list(job_ptr) \n");
			rebuild_job_part_list(job_ptr);
			job_cnt++;
			if (is_job_array_head &&
			    (job_ptr->array_task_id != NO_VAL)) {
				/* Try starting another task of the job array */
            info("cao: line 1505, Try starting another task of the job array: job_ptr = find_job_record(job_ptr->array_job_id);");
				job_ptr = find_job_record(job_ptr->array_job_id);
				if (job_ptr && IS_JOB_PENDING(job_ptr))
            {
               if (caolog) printf("cao:   jobscheduler.c, line 1509, if (job_ptr && IS_JOB_PENDING(job_ptr)) goto next_task\n");
					goto next_task;
            }
			}
			continue;
		...
		}// end of if (error_code == ESLURM_NODES_BUSY) {} else ...
      ...
      // info("cao: job_scheduler.c, int schedule(), line 531 ending the while(1)");      
	}// end the while(1)
...
#endif
   info("job_scheduler.c, end int schedule(), return job_cnt = %u)", job_cnt);
   info("=====================");
	return job_cnt;
}
...
{
   int caolog = 0;
   if (caolog) printf("cao:   job_scheduler.c, start sort_job_queue, do list_sort(job_queue, sort_job_queue2)\n");
	list_sort(job_queue, sort_job_queue2);
   if (caolog) printf("cao:   job_scheduler.c, end sort_job_queue\n");
}
...
{
   int caolog = 0;
   if (caolog) printf("cao:   job_scheduler.c, starting sort_job_queue2(void *x, void *y)\n");
						    ...
						    uint16_t protocol_version)
{
	...
	if ((launch_msg_ptr->script = get_job_script(job_ptr)) == NULL) {
      ...
      // info("cao: job_scheduler.c, line 1631, (void) job_complete()");
	...
	if (slurm_getpwuid_r(launch_msg_ptr->uid,
		...
		slurm_free_job_launch_msg(launch_msg_ptr);
      // info("cao: job_scheduler.c, line 1650, (void) job_complete()");
	...
	if (make_batch_job_cred(launch_msg_ptr, job_ptr, protocol_version)) {
      ...
      // info("cao: job_scheduler.c, line 1677, (void) job_complete()");
		(void) job_complete(job_ptr->job_id, getuid(), true, false, 0);
		return NULL;
	}
	...
	if (launch_msg_ptr->environment == NULL) {
		...
		slurm_free_job_launch_msg(launch_msg_ptr);
      // info("cao: job_scheduler.c, line 1705, (void) job_complete()");
...
extern int update_job_dependency(struct job_record *job_ptr, char *new_depend)
{
	...
	while (rc == SLURM_SUCCESS) {
		...
		if ((sep_ptr == NULL) && (tok[0] >= '0') && (tok[0] <= '9')) {
            ...
            info("cao:line 2205, array_task_id == NO_VAL, dep_job_ptr = find_job_record(job_id)");
		...
		while (rc == SLURM_SUCCESS) {
			...
			if (array_task_id == NO_VAL) {
            info("line 2285, cao: array_task_id == NO_VAL, dep_job_ptr = find_job_record(job_id)");
				dep_job_ptr = find_job_record(job_id);
				if (!dep_job_ptr) {
					dep_job_ptr = find_job_array_rec(job_id,
								      INFINITE);
...
{
   info("do int job_start_data(job_desc_msg_t *job_desc_msg, will_run_response_msg_t **resp)");
   info("Determine if a pending job will run using only the specified nodes");
	struct job_record *job_ptr;
	struct part_record *part_ptr;
   ...
   // info("cao: line 2507, job_start_data() job_ptr = find_job_record(job_desc_msg->job_id);");
...
{
   int caolog = 0;
   if (caolog) printf("cao: do _run_epilog(void *arg)\n");
	/* Locks: Write job */
	slurmctld_lock_t job_write_lock = {
 ...
 fini:	lock_slurmctld(job_write_lock);
   if (caolog) printf("cao:   do job_ptr = find_job_record(epilog_arg->job_id);\n");
	...
	xfree(epilog_arg);
   if (caolog) printf("cao:   end _run_epilog(void *arg)\n");
	return NULL;
}
...
static void *_run_prolog(void *arg)
{
	...
	argv[0] = xstrdup(slurmctld_conf.prolog_slurmctld); // cao: prolog_slurmctld = PrologSlurmctld =/etc/slurm/prologue
	argv[1] = NULL;
   ...
   /*
      printf("cao: argv[0]=%s\n", argv[0]);
      printf("cao: argv[1]=%s\n", argv[1]);
      printf("cao: my_env[0]=%s\n", my_env[0]);
      printf("cao: my_env[1]=%s\n", my_env[1]);
      printf("cao: my_env[2]=%s\n", my_env[2]);
      printf("cao: my_env[3]=%s\n", my_env[3]);
      printf("cao: my_env[4]=%s\n", my_env[4]);
      printf("cao: my_env[5]=%s\n", my_env[5]);
      */
 ...
 */
void cleanup_completing(struct job_record *job_ptr)
{
   //int requiredNode = job_ptr->total_cpus/8;
   //double required_pw = pw_one_cpu_socket * requiredNode;
   // printf("cao:  job_scheduler.c, void cleanup_completing(), JobId=%u(%iCPU)\n", job_ptr->job_id, job_ptr->total_cpus); 
	...
	if (delay > 60) {
		info("cleanup_completing(struct job_record *job_ptr): %s: job %u completion process took %ld seconds", __func__, job_ptr->job_id,(long) delay);
	}
   else{
		//info("do cleanup_completing(struct job_record *job_ptr): %s: job %u completion process took %ld seconds",  __func__, job_ptr->job_id,(long) delay);
	}
