...
{
   // printf("cao:  proc_req.c, line 1782, _slurm_rpc_complete_job_allocation(slurm_msg_t * msg)"); 
	static int active_rpc_cnt = 0;
	int error_code = SLURM_SUCCESS;
	DEF_TIMERS;
...
{
   // printf("cao:  proc_req.c, line 1873, _slurm_rpc_complete_batch_script(slurm_msg_t * msg)");
	static int active_rpc_cnt = 0;
...
{
   // printf("cao:  proc_req.c, line 2896, _slurm_rpc_step_complete(slurm_msg_t *msg)");
	static int active_rpc_cnt = 0;
...
{
   uid_t uidcao = g_slurm_auth_get_uid(msg->auth_cred, NULL);
      char strTimeNow[255];
      struct tm *ptr;
      time_t lt = time(NULL);
      ptr = localtime(&lt);
   ...
   // printf("cao:  proc_req.c, line 3083, starting the void _slurm_rpc_submit_batch_job(), UserID=%u at %s", uidcao, strTimeNow);       
   //info(strTimeNow);
	...
	if (error_code == SLURM_SUCCESS) {
		/* Locks are for job_submit plugin use */
		lock_slurmctld(job_read_lock);
      // printf("cao:  proc_req.c, line 3110,  validate_job_create_req()");
		error_code = validate_job_create_req(job_desc_msg,uid,&err_msg);
		unlock_slurmctld(job_read_lock);
	}
	...
	dump_job_desc(job_desc_msg);
	if (error_code == SLURM_SUCCESS) {
      // printf("cao:  proc_req.c, line 3116,  if (error_code == SLURM_SUCCESS)");
		_throttle_start(&active_rpc_cnt);
		lock_slurmctld(job_write_lock);
		...
		if (job_ptr) {	/* Active job allocation */
         //// printf("cao:  proc_req.c, line 3151,  if (job_ptr) /*Active job allocation*/ JobID=%u", job_ptr->job_id);
         //// printf("cao:  proc_req.c, line 3151,  if (job_ptr) /*Active job allocation*/ ");
#if defined HAVE_FRONT_END && !defined HAVE_BGQ	&& !defined HAVE_ALPS_CRAY
			/* Limited job step support */
         ...
         //// printf("cao:  proc_req.c, line 3174, do _launch_batch_step(job_desc_msg, JobID=%u", job_ptr->job_id); 
      ...
      // printf("cao:  proc_req.c, line 3213,  end of if (job_ptr) /*Active job allocation*/");//, JobID=%u", job_ptr->job_id); 
      //// printf("cao:  proc_req.c, line 3214,  error_code = job_allocate(job_desc_msg, ...) /* Create new job allocation */"); 
      ...
      // printf("cao:  proc_req.c, line 3216,  end of if (error_code == SLURM_SUCCESS) at line 3116"); 
	}
	...
	if ((error_code != SLURM_SUCCESS) &&
		...
		// printf("cao:  proc_req.c, line 3227, end of the void _slurm_rpc_submit_batch_job(), _slurm_rpc_submit_batch_job JobId=%u %s, job_ptr->comment = '%s'", job_ptr->job_id, TIME_STR, job_ptr->comment);
...
fini:	xfree(err_msg);
   //// printf("cao:  proc_req.c, end of void _slurm_rpc_submit_batch_job() at line 3069");
   char strTimeNow1[255];
      //struct tm *ptr;
      lt = time(NULL);
      ptr = localtime(&lt);
      strftime(strTimeNow1, 255, "%Y%m%d:%H:%M:%S", ptr);
      
      struct timeval nowt1;
      gettimeofday(&nowt1, NULL);
      sprintf(strTimeNow1, "%s.%i", strTimeNow1, (int)(1000000*((double) nowt1.tv_usec/1000000)));
      
   //info(strTimeNow);
   FILE *f = fopen("/tmp/cao_slurm.log", "a+");
   printf("%s  proc_req.c           (_slurm_rpc_submit_batch_job(%s)). JobId=%u, #CPUs=%u, job is submitted\n", strTimeNow1, strTimeNow, job_ptr->job_id, job_ptr->total_cpus);
   fprintf(f, "%s  proc_req.c           (_slurm_rpc_submit_batch_job()). JobId=%u, #CPUs=%u, job is submitted\n", strTimeNow1, job_ptr->job_id, job_ptr->total_cpus);
   fclose(f);
   // printf("cao:  proc_req.c, line 3252, end of the void _slurm_rpc_submit_batch_job(%s) JobId=%u %s at %s, job_ptr->comment = '%s'", strTimeNow, job_ptr->job_id, TIME_STR, strTimeNow1, job_ptr->comment);
}
