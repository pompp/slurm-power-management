		  ...
		  bool test_only, List *preemptee_job_list)
{
   int caolog = 0;
	...
	if (job_ptr->details->feature_list &&
      ...
      if (caolog) printf("cao: node_scheduler.c, int _get_req_features() Accumulate nodes with required feature counts (if (job_ptr->details->feature_list && ...), JobId=%u NodeList=%s #CPUs=%u\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus);                   
		...
		while ((feat_ptr = (struct feature_record *)
			...
			error_code = _pick_best_nodes(tmp_node_set_ptr,
					...
					exc_core_bitmap, resv_overlap);
         char *tmp_str1 = bitmap2node_name(feature_bitmap);
         if (caolog) printf("cao: node_scheduler.c, int _get_req_features() after _pick_best_nodes(), JobId=%u NodeList=%s #CPUs=%u, error_code = %i, feature_bitmap=%s\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, error_code, tmp_str1 );            
         xfree(tmp_str1);
#if 0
{
   ...
   if (caolog) printf("cao: node_scheduler.c, int _get_req_features() restore most of job state and accumulate remaining resource, JobId=%u NodeList=%s #CPUs=%u\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus);                   
	/* restore most of job state and accumulate remaining resources */
	if (saved_req_node_bitmap) {
      printf("cao: node_scheduler.c, int _get_req_features() restore most of job state and accumulate remaining resource (if (saved_req_node_bitmap)), JobId=%u NodeList=%s #CPUs=%u\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus);                   
		FREE_NULL_BITMAP(job_ptr->details->req_node_bitmap);
		job_ptr->details->req_node_bitmap =
				bit_copy(saved_req_node_bitmap);
	}
	if (accumulate_bitmap) {
      if (caolog) printf("cao: node_scheduler.c, int _get_req_features() restore most of job state and accumulate remaining resource (if (accumulate_bitmap)), JobId=%u NodeList=%s #CPUs=%u\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus);                   
		uint32_t node_cnt;
		if (job_ptr->details->req_node_bitmap) {
      ...
      if (caolog) printf("cao: node_scheduler.c, int _get_req_features() restore most of job state and accumulate remaining resource (if (accumulate_bitmap) == FALSE), JobId=%u NodeList=%s #CPUs=%u\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus);                      
...
#if 0
{
	...
	FREE_NULL_BITMAP(exc_core_bitmap);
   if (caolog) printf("cao: node_scheduler.c, ending of int _get_req_features(), JobId=%u NodeList=%s #CPUs=%u, return value = %i\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, error_code);            
...
{
   // printf("cao: node_scheduler.c, starting int _pick_best_nodes(), line 1052, JobId=%u NodeList=%s #CPUs=%u, **select_bitmap=%s\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, select_bitmap);                   
	struct node_record *node_ptr;
	...
	if ((job_ptr->details->min_nodes == 0) &&
		...
		if (pick_code == SLURM_SUCCESS) {
         ...
         // printf("cao: node_scheduler.c, int _pick_best_nodes(), line 1087, JobId=%u NodeList=%s #CPUs=%u, **select_bitmap=%s return SLURM_SUCCESS\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, select_bitmap);                   
			return SLURM_SUCCESS;
		} else {
			bit_free(avail_bitmap);
         // printf("cao: node_scheduler.c, int _pick_best_nodes(), line 1091, JobId=%u NodeList=%s #CPUs=%u, **select_bitmap=%s return ESLURM_REQUESTED_NODE_CONFIG_UNAVAILABLE\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, select_bitmap);                   
   ...
   // printf("cao: node_scheduler.c, starting int _pick_best_nodes(), line 1100, JobId=%u NodeList=%s #CPUs=%u, **select_bitmap=%s, 'Are Consumable Resources enabled?  Check once'\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, select_bitmap);                   
	if (cr_enabled == NO_VAL) {
		...
		if (error_code != SLURM_SUCCESS) {
			cr_enabled = NO_VAL;
         // printf("cao: node_scheduler.c, int _pick_best_nodes(), line 1106, JobId=%u NodeList=%s #CPUs=%u, **select_bitmap=%s return error_code=%i\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, select_bitmap, error_code);                   
			return error_code;
		}
	}
	...
	if (job_ptr->details->req_node_bitmap) {  /* specific nodes required */
      ...
      // printf("cao: node_scheduler.c, int _pick_best_nodes(), line 1152, JobId=%u NodeList=%s #CPUs=%u, **select_bitmap=%s, 'check the availability of these nodes'\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, select_bitmap);                   
      ...
      // printf("cao: node_scheduler.c, int _pick_best_nodes(), line 1190, JobId=%u NodeList=%s #CPUs=%u, **select_bitmap=%s, 'still must go through select_g_job_test() '\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, select_bitmap);                   
   ...
   // printf("cao: node_scheduler.c, int _pick_best_nodes(), line 1211, JobId=%u NodeList=%s #CPUs=%u, **select_bitmap=%s, 'Accumulate resources for this job based upon its required feature '\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, select_bitmap);                    
	for (j = min_feature; j <= max_feature; j++) {
      // printf("cao: node_scheduler.c, int _pick_best_nodes(), line 1213, JobId=%u NodeList=%s #CPUs=%u, **select_bitmap=%s, 'j = %i\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, select_bitmap, j);                    
		if (job_ptr->details->req_node_bitmap) {
...
#if 0
{
      ...
      // printf("cao: node_scheduler.c, int _pick_best_nodes(), line 1361, JobId=%u NodeList=%s #CPUs=%u, **select_bitmap=%s try to get req_nodes now for this feature\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, select_bitmap);                   
		if (avail_bitmap && (!tried_sched)	&&
			...
			if ((pick_code == SLURM_SUCCESS) &&
				...
				FREE_NULL_BITMAP(possible_bitmap);
				*select_bitmap = avail_bitmap;
            // printf("cao: node_scheduler.c, int _pick_best_nodes() line 1383, JobId=%u NodeList=%s #CPUs=%u, **select_bitmap=%s\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, select_bitmap);                   
				return SLURM_SUCCESS;
			}
		}
		 ...
		 * nodes available) */
      // printf("cao: node_scheduler.c, int _pick_best_nodes(), line 1390, JobId=%u NodeList=%s #CPUs=%u, **select_bitmap=%s (determine if job could possibly run (if all configured nodes available)\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, select_bitmap);                    
		if (total_bitmap)
   ...
   // printf("cao: node_scheduler.c, int _pick_best_nodes(), line 1448, JobId=%u NodeList=%s #CPUs=%u, **select_bitmap=%s\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, select_bitmap);                    
	/* The job is not able to start right now, return a
	 * value indicating when the job can start */
	if (!runable_ever && resv_overlap) {
		error_code = ESLURM_RESERVATION_BUSY;
      // printf("cao: node_scheduler.c, int _pick_best_nodes(), line 1453, JobId=%u NodeList=%s #CPUs=%u, **select_bitmap=%s (The job is not able to start right now)\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, select_bitmap);                    
		return error_code;
	}
	if (!runable_ever) {
		error_code = ESLURM_REQUESTED_NODE_CONFIG_UNAVAILABLE;
      ...
      // printf("cao: node_scheduler.c, int _pick_best_nodes(), line 1462, JobId=%u NodeList=%s #CPUs=%u, **select_bitmap=%s (return error_code = ESLURM_NODE_NOT_AVAIL)\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, select_bitmap);                    
   ...
   // printf("cao: node_scheduler.c, int _pick_best_nodes() line 1460, JobId=%u NodeList=%s #CPUs=%u, **select_bitmap=%s\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, select_bitmap);                   
	return error_code;
}
 ...
 */
// call on the job_scheduler: error_code = select_nodes(job_ptr, false, NULL, NULL);

extern int select_nodes(struct job_record *job_ptr, bool test_only,
			bitstr_t **select_node_bitmap, char **err_msg)
{
   int caolog = 0;
   if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1561, starting int select_nodes() JobId=%u NodeList=%s #CPUs=%u\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus); 
   ...
   if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1655, return ESLURM_REQUESTED_NODE_CONFIG_UNAVAILABLE = %i\n", ESLURM_REQUESTED_NODE_CONFIG_UNAVAILABLE);
	//return ESLURM_REQUESTED_NODE_CONFIG_UNAVAILABLE;
   
	/* build sets of usable nodes based upon their configuration */
   if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1627, before _build_node_list, JobId=%u NodeList=%s #CPUs=%u\n",  job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus);    
	error_code = _build_node_list(job_ptr, &node_set_ptr, &node_set_size,
      ...
      time_t lt = time(NULL);
      ptr = localtime(&lt);
      strftime(strTimeNow, 255, "%Y%m%d:%H:%M:%S", ptr);
      struct timeval nowt;
      gettimeofday(&nowt, NULL);
      sprintf(strTimeNow, "%s.%i", strTimeNow, (int)(1000000*((double) nowt.tv_usec/1000000)));

   if (error_code){
      if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1648, return error_code = %i, timenow = %s\n", error_code, strTimeNow);
		return error_code;
   }

	/* insure that selected nodes are in these node sets */
   if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1652, insure that selected nodes are in these node sets\n");
	if (job_ptr->details->req_node_bitmap) {
	...
	*/
   if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1681, Don't call functions in MIN/MAX it will result in the ... \n");
   ...
   // error tu day
	if (acct_max_nodes < min_nodes) {
      ...
      if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1718, startjob_array_pre_sched(job_ptr): JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);
		...
		error_code = _get_req_features(node_set_ptr, node_set_size,
					       ...
					       req_nodes, test_only,
					       &preemptee_job_list);
      if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1725, ended _get_req_features(): ..., JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);                      
	}
	 ...
	 */
   if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1734, Set this guess here to give the user tools an idea..., JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);
	if (select_bitmap
   ...
   if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1758, set up the cpu_cnt here so we can decrement it as nodes free up..., JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);
	 ...
	 * memory. */
   if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1834, This job may be getting requeued, ... JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);
	...
	if ((job_ptr->time_limit == NO_VAL) ||
      ...
      if (caolog) ("cao: node_scheduler.c, int select_nodes() line 1850, if ((job_ptr->time_limit == NO_VAL) , ... JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);
	...
	if (select_g_job_begin(job_ptr) != SLURM_SUCCESS) {
      if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1863, if (select_g_job_begin(job_ptr) != SLURM_SUCCESS), ... JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);
	...
	if (job_ptr->job_resrcs && job_ptr->job_resrcs->nodes){
      if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1879, if (job_ptr->job_resrcs && job_ptr->job_resrcs->nodes)      ,JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);
		job_ptr->nodes = xstrdup(job_ptr->job_resrcs->nodes);
      if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1882, ended   job_ptr->nodes = xstrdup(job_ptr->job_resrcs->nodes),JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);      
   }
	else {
   ...
   if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1927, Job is starting here, allocate_nodes(job_ptr), job_array_start(job_ptr), job_array_start(job_ptr) build_node_details(job_ptr, true); rebuild_job_part_list(job_ptr);\n");
   if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1887, calling allocate_nodes(job_ptr)       ,JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);
	allocate_nodes(job_ptr);
   
   if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1889, ended allocate_nodes(job_ptr)         ,JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);
   if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1890, calling job_array_start(job_ptr)      ,JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);
	job_array_start(job_ptr);
   
   if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1892, ended job_array_start(job_ptr)        ,JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);
   if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1893, calling build_node_details(job_ptr)   ,JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);
	build_node_details(job_ptr, true);
   
   if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1895, ended build_node_details(job_ptr)     ,JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);
	rebuild_job_part_list(job_ptr);
   
   if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1896, ended rebuild_job_part_list(job_ptr)  ,JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);

	/* This could be set in the select plugin so we want to keep the flag */
   if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1889, This could be set in the select plugin so we want to keep the flag, JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);
   ...
   if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1930, Request asynchronous launch of a prolog for a non batch job, JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);
	...
	if (job_ptr->array_recs && job_ptr->array_recs->task_id_bitmap &&
      ...
      if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1940, job_ptr->array_task_id = NO_VAL, JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);
	}
	if (preemptee_job_list){
		list_destroy(preemptee_job_list);
      if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1944, list_destroy(preemptee_job_list), JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);
      }
	if (select_node_bitmap){
		*select_node_bitmap = select_bitmap;
      if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1948, *select_node_bitmap = select_bitmap, JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);
      }
	else{
		FREE_NULL_BITMAP(select_bitmap);
      if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1952, FREE_NULL_BITMAP(select_bitmap), JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);
      }
	if (node_set_ptr) {
      if (caolog) printf("cao: node_scheduler.c, int select_nodes() line 1955, if (node_set_ptr != NULL), JobId=%u NodeList=%s #CPUs=%u at %s\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow);
		for (i = 0; i < node_set_size; i++) {
   ...
   if (caolog) printf("slurmctld/node_scheduler.c, int select_nodes(), ended, JobId=%u NodeList=%s #CPUs=%u at %s, return value = %i\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus, strTimeNow,error_code);
 ...
 * RET error code
 */
//error_code = _build_node_list(job_ptr, &node_set_ptr, &node_set_size, err_msg);
static int _build_node_list(struct job_record *job_ptr,
			    struct node_set **node_set_pptr,
			    int *node_set_size, char **err_msg)
{
   ...
   if (caolog) {
      if (job_ptr->job_resrcs && job_ptr->job_resrcs->nodes)
         printf("slurmctld/node_scheduler.c, int _build_node_list() starting the int _build_node_list() line 2382, JobId=%u job_ptr->job_resrcs->nodes = %s NodeList=%s #CPUs=%u\n", job_ptr->job_id, job_ptr->job_resrcs->nodes, job_ptr->nodes, job_ptr->total_cpus);
      else    
         printf("slurmctld/node_scheduler.c, int _build_node_list() starting the int _build_node_list() line 2382, JobId=%u job_ptr->job_resrcs->nodes = NULL NodeList=%s #CPUs=%u\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus);
	}
   int adj_cpus, i, node_set_inx, power_cnt, rc;
	...
	if (job_ptr->resv_name) /* reservation name */ {
		/* Limit node selection to those in selected reservation */
      if (caolog) printf("slurmctld/node_scheduler.c, int _build_node_list():   if (job_ptr->resv_name) line 2352\n");
		time_t start_res = time(NULL);
		...
		if ((detail_ptr->req_node_bitmap) &&
		    (!bit_super_set(detail_ptr->req_node_bitmap,
				    usable_node_mask))) {
         if (caolog) printf("slurmctld/node_scheduler.c, int _build_node_list(): if ((detail_ptr->req_node_bitmap) && ...) line 2374\n");       
			job_ptr->state_reason = WAIT_RESERVATION;
			xfree(job_ptr->state_desc);
	...
	if ((job_ptr->details->min_nodes == 0) &&
      ...
      if (caolog){
         if (job_ptr->job_resrcs && job_ptr->job_resrcs->nodes)
            printf("slurmctld/node_scheduler.c, int _build_node_list(): (if ((job_ptr->details->min_nodes == 0) && ...), JobId=%u job_ptr->job_resrcs->nodes = %s NodeList=%s #CPUs=%u\n", job_ptr->job_id, job_ptr->job_resrcs->nodes, job_ptr->nodes, job_ptr->total_cpus);
         else
            printf("slurmctld/node_scheduler.c, int _build_node_list(): (if ((job_ptr->details->min_nodes == 0) && ...), JobId=%u job_ptr->job_resrcs->nodes = NULL NodeList=%s #CPUs=%u\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus);
		}
      return SLURM_SUCCESS;
	}
   ...
   if (caolog) printf("cao: node_scheduler.c, int _build_node_list(): starting the while ((config_ptr = (struct config_record *) list_next(config_iterator))), JobID=%u timenow=%s\n", job_ptr->job_id, strTimeNow);
   // may be power should be added here   
	while ((config_ptr = (struct config_record *)
			list_next(config_iterator))) {
      if (caolog) printf("cao: node_scheduler.c, int _build_node_list(): checking the node_set_ptr[node_set_inx =%i], line 2431\n", node_set_inx);         
		...
		adj_cpus = adjust_cpus_nppcu(_get_ntasks_per_core(detail_ptr),
					     config_ptr->threads,
					     config_ptr->cpus);
      if (caolog) printf("cao: node_scheduler.c, int _build_node_list(): node_set_inx = %i: adjust_cpus_nppcu(): adj_cpus = %i (max cpu on this node)\n", node_set_inx, adj_cpus);
      if (caolog) printf("cao: node_scheduler.c, int _build_node_list(): node_set_inx = %i: detail_ptr->pn_min_cpus[%i] <= adj_cpus [%i]\n", node_set_inx, detail_ptr->pn_min_cpus, adj_cpus);
      if (caolog) printf("cao: node_scheduler.c, int _build_node_list(): node_set_inx = %i: detail_ptr->pn_min_memory[%i] <= config_ptr->real_memory [%i]\n", node_set_inx, detail_ptr->pn_min_memory, config_ptr->real_memory);
      if (caolog) printf("cao: node_scheduler.c, int _build_node_list(): node_set_inx = %i: detail_ptr->pn_min_tmp_disk[%i] <= config_ptr->tmp_disk [%i]\n", node_set_inx, detail_ptr->pn_min_tmp_disk, config_ptr->tmp_disk);
   ...
   if (job_ptr->job_resrcs && job_ptr->job_resrcs->nodes)
   {
       if (caolog) printf("cao: node_scheduler.c, int _build_node_list(): end the while((config_ptr = ...) line 2585, JobId=%u job_ptr->job_resrcs->nodes = %s NodeList=%s #CPUs=%u\n", job_ptr->job_id, job_ptr->job_resrcs->nodes, job_ptr->nodes, job_ptr->total_cpus);
   }
   else
   {
      if (caolog) printf("cao: node_scheduler.c, int _build_node_list(): end the while((config_ptr = ...) line 2585, JobId=%u job_ptr->job_resrcs->nodes = NULL NodeList=%s #CPUs=%u\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus);
   }
   ...
   if (caolog) printf("cao: node_scheduler.c, int _build_node_list(): If any nodes are powered down, put them into a new node_set ... line 2561\n"); 
	for (i = (node_set_inx-1); i >= 0; i--) {
		power_cnt = bit_overlap(node_set_ptr[i].my_bitmap,
					power_node_bitmap);
		if (power_cnt == 0) {
         if (caolog) printf("cao: node_scheduler.c, int _build_node_list(): node_set_inx[%i]: no nodes powered down, line 2637\n", i); 
			continue;	/* no nodes powered down */
      }
		if (power_cnt == node_set_ptr[i].nodes) {
			node_set_ptr[i].weight += max_weight;	/* avoid all */
         if (caolog) printf("cao: node_scheduler.c, int _build_node_list(): node_set_inx[%i]: all nodes powered down, line 2642\n", i); 
			continue;	/* all nodes powered down */
		}
      if (caolog) printf("cao: node_scheduler.c, int _build_node_list(): Some nodes powered down, others up, split record line 2645\n"); 
   ...
   if (job_ptr->job_resrcs && job_ptr->job_resrcs->nodes){
      if (caolog) printf("cao: node_scheduler.c, int _build_node_list(): finishing, line 2679, JobId=%u job_ptr->job_resrcs->nodes = %s NodeList=%s #CPUs=%u\n", job_ptr->job_id, job_ptr->job_resrcs->nodes, job_ptr->nodes, job_ptr->total_cpus);
   }
   else{
      if (caolog) printf("cao: node_scheduler.c, int _build_node_list(): finishing, line 2681, JobId=%u job_ptr->job_resrcs->nodes = NULL NodeList=%s #CPUs=%u\n", job_ptr->job_id, job_ptr->nodes, job_ptr->total_cpus);
   }
   if (caolog) printf("cao: slurmctld/node_scheduler.c, int _build_node_list(): return SLURM_SUCCESS = %i, node_set_inx = %i, line 2683\n", SLURM_SUCCESS, node_set_inx); 
	return SLURM_SUCCESS;
}
