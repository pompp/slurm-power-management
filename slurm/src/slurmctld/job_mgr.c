			...
			bool node_fail, uint32_t job_return_code)
{
   ...
   printf("%s  job_mrg.c            (int job_complete()).            JobId=%u, #CPUs=%u, job is completed\n", strTimeNow, job_ptr->job_id, job_ptr->total_cpus);   
   fprintf(f, "%s  job_mrg.c            (int job_complete()).            JobId=%u, #CPUs=%u, job is completed\n", strTimeNow, job_ptr->job_id, job_ptr->total_cpus);
   fclose(f);  
   
	//printf("cao: %s: %s done", __func__, jobid2str(job_ptr, jbuf));
...
void job_time_limit(void)
{
	...
	while ((job_ptr =(struct job_record *) list_next(job_iterator))) {
		...
		if (job_ptr->time_limit != INFINITE) {
            ...
            FILE *f = fopen("/tmp/cao_slurm.log", "a+");
            printf("%s  job_mrg.c            (void job_time_limit()). Time exhausted JobId=%u, #CPUs=%u, job is terminated\n", strTimeNow, job_ptr->job_id, job_ptr->total_cpus);   
            fprintf(f, "%s  job_mrg.c            (void job_time_limit()). Time exhausted JobId=%u, #CPUs=%u, job is terminated\n", strTimeNow, job_ptr->job_id, job_ptr->total_cpus);   
            fclose(f);  
...
jobid2str(struct job_record *job_ptr, char *buf)
{
	...
	if (job_ptr->array_recs && (job_ptr->array_task_id == NO_VAL)) {
		...
		sprintf(buf, "cao: slurmctld/job_mgr.c, jobid2str(): JobID=%u State=0x%x NodeCnt=%u at %s",
			job_ptr->job_id, job_ptr->job_state,
			job_ptr->node_cnt, strTimeNow);
	} else {
		sprintf(buf, "cao: slurmctld/job_mgr.c, jobid2str(): JobID=%u_%u(%u) State=0x%x NodeCnt=%u at %s",
			job_ptr->array_job_id, job_ptr->array_task_id,
			job_ptr->job_id, job_ptr->job_state,job_ptr->node_cnt, strTimeNow);
	}
...
trace_job(struct job_record *job_ptr, const char *func, const char *extra)
{
	...
	if (slurmctld_conf.debug_flags & DEBUG_FLAG_TRACE_JOBS)
		info("cao: trace_job(): %s: %s job %s", func, extra, jobid2str(job_ptr, jbuf));
}
