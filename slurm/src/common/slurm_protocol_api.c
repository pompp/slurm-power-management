...
{
   int caolog = 0;
   if (caolog) printf("cao: src/common/slurm_protocol_api.c int slurm_send_recv_controller_msg()\n");
   if (caolog) printf("                                   opens a connection to the controller, sends the controller a message, listens for the response, then closes the connection\n");
	slurm_fd_t fd = -1;
	int rc = 0;
	time_t start_time = time(NULL);
	int retry = 1;
