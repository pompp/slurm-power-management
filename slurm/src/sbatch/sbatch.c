...
{
   int caolog = 0;
   if (caolog) printf("cao: sbatch.c: int main() argv[0] = %s, argv[1] = %s, argv[2] = %s, argv[3] = %s, argv[4] = %s, argv[5] = %s, line 82\n", argv[0], argv[1], argv[2], argv[3], argv[4], argv[5]);
   ...
   if (caolog) printf("cao: sbatch.c: line 99\n");
   ...
   if (caolog) printf("cao: sbatch.c: line 106\n");
   ...
   if (caolog) printf("cao: sbatch.c: line 114\n");
	if (opt.wrap != NULL) {
		script_body = _script_wrap(opt.wrap);
	} else {
		script_body = _get_script_buffer(script_name, &script_size);
	}
   if (caolog) printf("cao: sbatch.c: line 120\n");
	if (script_body == NULL)
		exit(error_exit);
   if (caolog) printf("cao: sbatch.c: line 123\n");
   ...
   if (caolog) printf("cao: sbatch.c: line 133\n");
	if (spank_init_post_opt() < 0) {
		error("Plugin stack post-option processing failed");
		exit(error_exit);
	}

   if (caolog) printf("cao: sbatch.c: line 139\n");
   ...
   if (caolog) printf("cao: sbatch.c: line 146\n");
	/*
   ...
   if (caolog) printf("cao: sbatch.c: line 163\n");

	desc.script = (char *)script_body;

	// If can run on multiple clusters find the earliest run time and run it there 
	if (sbatch_set_first_avail_cluster(&desc) != SLURM_SUCCESS)
		exit(error_exit);

   if (caolog) printf("cao: sbatch.c: line 172\n");
	if (_check_cluster_specific_settings(&desc) != SLURM_SUCCESS)
		exit(error_exit);

   if (caolog) printf("cao: sbatch.c: line 176\n");
	if (opt.test_only) {
   ...
   // cao: src/api/submit.c: slurm_submit_batch_job (job_desc_msg_t *req, submit_response_msg_t **resp)
   // in 
   if (caolog) printf("cao: sbatch.c: starting the 'while (slurm_submit_batch_job(&desc, &resp) < 0) ' %u, line 179\n", resp->job_id);
   // cao: submit job is in the slurm_submit_batch_job(&desc, &resp)
	while (slurm_submit_batch_job(&desc, &resp) < 0) {
      if (caolog) printf("cao: sbatch.c: inside the 'while (slurm_submit_batch_job(&desc, &resp) < 0) ' %u, line 181\n", resp->job_id);
		static char *msg;
	...
	if (!opt.parsable){
		printf("sbatch.c: Submitted batch job %u\n", resp->job_id);
		if (working_cluster_rec)
			printf(" on cluster %s\n", working_cluster_rec->name);
         
	} else {
		if (caolog) printf("cao: sbatch.c: Submitted batch job, line 211\n");
		if (working_cluster_rec)
			if (caolog) printf("cao: sbatch.c: Submitted batch job, line 213, working_cluster_rec->name = %s\n", working_cluster_rec->name);
	}
   
   if (caolog) printf("cao: sbatch.c: end of the main(), line 229\n");
	xfree(desc.script);
